  ...
  return (
    <section id="experience" className="py-28 px-6">
      <div className="max-w-5xl mx-auto">
        <FadeUp>
          <p className="text-xs text-violet-400 font-mono uppercase tracking-widest mb-2">Work</p>
          <h2 className="text-4xl font-bold text-white mb-16">Industry Experience</h2>
        </FadeUp>

        <div className="relative">
          {/* Timeline line */}
          <div className="absolute left-0 md:left-8 top-0 bottom-0 w-px bg-gradient-to-b from-violet-500/50 via-indigo-500/30 to-transparent hidden md:block" />
            ...
            {JOBS.map((job, ji) => (
              <FadeUp key={job.company} delay={ji * 0.1}>
                <div className="md:pl-24 relative">
                  {/* Timeline dot */}
                  <div className={`hidden md:flex absolute left-5 top-1 w-6 h-6 rounded-full bg-gradient-to-br ${job.accent} items-center justify-center shadow-[0_0_20px_rgba(139,92,246,0.4)]`}>
                  ...
                  <Stagger className="space-y-4">
                    {job.projects.map(p => (
                      <motion.div key={p.name} variants={itemVariants}
                        className="card-base p-5 group hover:translate-y-[-2px] transition-transform duration-300">
                        <div className="flex flex-wrap items-start justify-between gap-3 mb-3">
                          <h4 className="text-white font-semibold text-sm">{p.name}</h4>
                          <span className="text-xs font-mono text-violet-400 bg-violet-500/10 px-2.5 py-1 rounded-full border border-violet-500/20 shrink-0">
                            {p.stat}
                          </span>
                        </div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {p.tags.map(t => (
                            <span key={t} className="text-xs font-mono px-2.5 py-1 rounded-md bg-white/[0.04] text-slate-400 border border-white/[0.06]">
                              {t}
                            </span>
                          ))}
                        </div>
